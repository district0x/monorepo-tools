...
(deftest migrate-library-tests
  (testing "checks library structure (deps.edn, shadow-cljs)"
    (is (false? (ml/library-structure-as-expected? (fixture-path "lib-no-deps"))))
    (is (true? (ml/library-structure-as-expected? (fixture-path "lib-ok")))))

  (testing "moves selected library to subfolder with git history"
    (let [temp-path (fs/create-temp-dir {:prefix temp-prefix})
          test-repo-path (set-up-git-repo (fixture-path "test-repo.tar.gz") temp-path)
          container-repo-path (set-up-git-repo (fixture-path "container-repo.tar.gz") temp-path)]
