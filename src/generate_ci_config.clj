  ...
  [(format "      - run:")
   ...
   (when (or (server-library? library) (shared-library? library))
     (vector
       ...
       (format "          command: cd %s && clojure -A:shadow-cljs:local-deps compile test-node" library)
       (format "      - run:")
   ...
   (when (or (browser-library? library) (shared-library? library))
     (vector
       ...
       (format "          command: cd %s && clojure -A:shadow-cljs:local-deps compile test-ci" library)
       (format "      - run:")
